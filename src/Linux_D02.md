...
***Remote forwarding** (`-R`) при подключении к `ws22`, нужно использовать, если мы хотим перенаправить порт с `ws11` (или другого клиента) на `ws22`, но в вашем сценарии мы используем его наоборот, что неправильно.*

*В нашем случае, использование `-L` на `ws21` для туннелирования доступа к веб-серверу на `ws22` — правильный подход.*

*Если мы подключаемся с использованием `-R` и у вас возникает ошибка `connection refused` при попытке использовать `telnet`, это потому, что `-R` перенаправляет порт на удаленной машине (`ws22`), а не на локальной (`ws11`). Следовательно, в нашем сценарии `-R` не дает нужного результата.*
