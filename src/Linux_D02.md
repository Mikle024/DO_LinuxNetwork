...
## 4. Сетевой экран:
- Создал файл /etc/firewall.sh, имитирующий фаерволл, на ws1 и ws2 и настроил удаление всех правил в таблице «filter» (по-умолчанию).
...
iptables -t filter -A INPUT -p tcp --dport 88 -j ACCEPT
...
- Разрешил доступ и запустил файлы на обеих машинах командами `chmod +x /etc/firewall.sh` и `/etc/firewall.sh`.

> Вывод команд `chmod +x /etc/firewall.sh` и `/etc/firewall.sh` (ws1):
>
> ![Вывод команд chmod +x /etc/firewall.sh и /etc/firewall.sh ws1](screen%2Fscreen_4_04.png)
>

> Вывод команд `chmod +x /etc/firewall.sh` и `/etc/firewall.sh` (ws2):
>
> ![Вывод команд chmod +x /etc/firewall.sh и /etc/firewall.sh ws2](screen%2Fscreen_4_05.png)
>
...
> Пинг без ответа ip 192.168.100.10:
>
...
* `nameserver 8.8.8.8` - эта строка добавляет дополнительный DNS-сервер, к которому система может обратиться для разрешения доменных имен. В данном случае, это **8.8.8.8** — **публичный DNS-сервер**, предоставляемый компанией **Google**.*

- Выполнил команду от имени администратора `sudo` `systemctl restart isc-dhcp-server`, что бы перезапустить систему **DHCP-сервера**.
- Выполнил команду от имени администратора `sudo` `systemctl status isc-dhcp-server`, что бы проверить статус работы системы.


> Успешно запущенная система **DHCP-сервера** (r2):
>
> ![Отредактированный файл /etc/resolv.conf r2](screen%2Fscreen_6_09.png)
>
...
> 
> Изменил значение `dhcp4` на `true`, и закомментировал настройки для статической выдачи ip.
> 
